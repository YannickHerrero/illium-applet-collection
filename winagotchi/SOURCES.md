...
`assets/sprites/*.png` are unchanged upstream artwork. Root-level PNGs are identical copies of the idle/sleep first frames, because Illium's dynamic bar icon field accepts only plain filenames in the applet directory. No sound files, playback code or audio settings are included. The Creative Commons sound assets are not redistributed.
