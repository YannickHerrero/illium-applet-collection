...
spec = importlib.util.spec_from_file_location('illigotchi_install', ROOT / 'install.py')
...
class InstallTests(unittest.TestCase):
    def setUp(self):
        ...
        self.binary = self.root / 'illigotchi.exe'; self.binary.write_bytes(image)
    ...
    def test_preserves_bar_and_installs_complete_silent_pack(self):
        ...
        self.assertEqual(data['left'], ['illigotchi', 'workspaces']); self.assertEqual(data['center'], ['clock'])
        ...
        self.assertEqual(manifest['command'], [r'C:\Users\Test User\.config\illium\applets\illigotchi\illigotchi.exe'])
        self.assertEqual((app / 'illigotchi.exe').read_bytes(), self.binary.read_bytes())
    ...
    def test_empty_unicode_and_quoted_brackets(self):
        for raw in [b'left=[]', 'left = ["日本語", "a]b"] # comment\n'.encode()]:
            ...
            self.assertEqual(result, dict(original, left=['illigotchi'] + original['left']))
    ...
    def test_inserts_immediately_before_workspaces_preserving_other_positions(self):
        ...
        result = tomllib.loads(install.plan_bar(raw).decode())
        self.assertEqual(result['left'], ['clock', 'separator', 'illigotchi', 'workspaces', 'weather'])
    ...
    def test_failure_rolls_back_applet_without_touching_bar(self):
        ...
        self.assertEqual(self.bar.read_bytes(), self.original)
        self.assertFalse((self.config / 'applets/illigotchi').exists())
        self.assertEqual(list(self.root.glob('.illigotchi-stage-*')), [])
