...
Colors come from the active Illium theme, so switching theme repaints the applet with the rest of the bar.
...
The installer refuses to overwrite an existing installation, backs up `bar.toml` under `~/.local/state/illium-applet-collection/backups/`, copies the applet to Windows `~/.config/illium/applets/todo/` and puts `todo` first in the bar's `drawer` array, behind the chevron. A bar without a drawer gets it in `right` instead, before `claude-usage`, or at the end when that module is absent. Pass `--config-home` for a `ILLIUM_CONFIG_HOME` override. To remove it, delete `todo` from the bar and remove the applet directory; the task file survives, since it lives outside the configuration.
