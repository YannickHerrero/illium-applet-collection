...
A read-only Winarchy applet listing every coding agent visible from WSL in one place: the agents running in [herdr](https://herdr.dev) panes and the tasks the [Multica](https://multica.ai) agents are working on. The bar tells you how many need an answer or are working; the popup lets you filter by state. Nothing in the popup focuses, cancels or kills anything.
...
The bar icon changes with the loudest state: three linked nodes at rest, filled nodes while agents work, one node replaced by an exclamation mark when an agent needs you or a task failed. The label is the number of agents needing you, otherwise the number working, otherwise empty.

The popup has a headline (**2 agents need you**, **3 agents working**, **1 task failed**, **7 agents idle**), one line per source with its reachability, then filter tabs (**All**, **Working**, **Needs you**, **Done**, **Idle**) and one card per agent:
...
- a detail line: the working directory and herdr workspace, or the Multica task kind, wait or failure reason and elapsed time
- the state: **needs you**, **working**, **queued**, **failed**, **done** or **ready**

States map onto each source's own words. Herdr: `blocked` is **needs you**, `done` means a pane finished while unfocused, `idle` reads **ready**. Multica: `running` is **working**; `queued`, `dispatched` and `deferred` are **queued**; `waiting_local_directory` is **needs you**; an agent's last `completed` or `failed` task is shown as **done** or **failed** for two hours after it finished, then the agent reads **ready**. Multica has no "question pending" task state: an agent that asks you something posts a comment and its task completes.

Cards are ordered needs you, working, queued, failed, done, ready. The list refreshes every 15 seconds while closed and every 3 seconds while open.
