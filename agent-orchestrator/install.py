#!/usr/bin/env python3
"""Install the Agent Orchestrator applet from WSL, preserving the existing bar configuration."""
import argparse
import datetime
import json
import os
...
def patched_bar(raw):
    ...
    if NAME in sum((data.get(k, []) for k in ('left', 'center', 'right')), []):
    ...
    match = matches[0]
    result = text[:match.start()] + 'right = ' + json.dumps(modules + [NAME]) + text[match.end():]
    if tomllib.loads(result) != dict(data, right=modules + [NAME]):
...
def patched_manifest(distribution, script):
    ...
    command = ['wsl.exe', '-d', distribution, '--', 'python3', str(script)]
